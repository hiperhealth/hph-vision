...
export const DEFAULT_CYLINDER_RANGE: [number, number] = [-4.00, 0.00];
...
export type AstigmatismPattern = 'clockDial' | 'fanChart' | 'lineOrientation' | 'jcc';
...
export const normalizeAnswer = (answer: AstigmatismAnswer): AstigmatismAnswer => ({
  ...answer,
  selectedAxis: answer.selectedAxis !== undefined ? normalizeAxis(answer.selectedAxis) : undefined,
  jccAxisA: answer.jccAxisA !== undefined ? normalizeAxis(answer.jccAxisA) : undefined,
  jccAxisB: answer.jccAxisB !== undefined ? normalizeAxis(answer.jccAxisB) : undefined,
});

export const generateClockDialPrompt = (): number[] => [30, 60, 90, 120, 150, 180];

export const generateFanChartPrompt = (): number[] =>
  Array.from({ length: 18 }, (_, i) => (i + 1) * 10);

export const generateLineOrientationPrompt = (currentAxisRange: [number, number]): number[] => {
...
export const generateJccPrompt = (currentAxisRange: [number, number]): [number, number] => {
  const mid = Math.round((currentAxisRange[0] + currentAxisRange[1]) / 2);
  return [normalizeAxis(mid - JCC_AXIS_SHIFT), normalizeAxis(mid + JCC_AXIS_SHIFT)];
};
...
export const isContradictoryAnswer = (
  ...
  if (answer.selectedAxis !== undefined) {
    if (answer.selectedAxis < min || answer.selectedAxis > max) {
      ...
      if (distMin > CONTRADICTION_THRESHOLD_AXIS && distMax > CONTRADICTION_THRESHOLD_AXIS) {
        return true;
      }
    }
  }
  ...
  if (answer.jccPreference === 'A' && answer.jccAxisA !== undefined) {
    if (answer.jccAxisA < min - CONTRADICTION_THRESHOLD_AXIS || answer.jccAxisA > max + CONTRADICTION_THRESHOLD_AXIS) return true;
  }

  if (answer.jccPreference === 'B' && answer.jccAxisB !== undefined) {
    if (answer.jccAxisB < min - CONTRADICTION_THRESHOLD_AXIS || answer.jccAxisB > max + CONTRADICTION_THRESHOLD_AXIS) return true;
  }
...
export const estimateAxisRange = (
  ...
  if (answer.pattern === 'clockDial' || answer.pattern === 'fanChart' || answer.pattern === 'lineOrientation') {
...
export const estimateCylinderRange = (
  ...
  if (answer.pattern === 'clockDial' || answer.pattern === 'fanChart' || answer.pattern === 'lineOrientation') {
    if (answer.selectedAxis !== undefined) {
      const cylMin = normalizeCylinder(Math.max(currentRange[0], -2.00));
      return cylMin <= currentRange[1] ? [cylMin, currentRange[1]] : [cylMin, cylMin];
    }
  } else if (answer.pattern === 'jcc') {
    if (answer.jccPreference === 'A' || answer.jccPreference === 'B') {
      const cylMax = normalizeCylinder(Math.min(currentRange[1], -0.25));
      return currentRange[0] <= cylMax ? [currentRange[0], cylMax] : [cylMax, cylMax];
